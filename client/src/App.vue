...
const openModal = () => {
  ...
  socket.on("response", (response) => {
    ...
    // if message vide alors appeler la fonction closeModal
    // if (response === '') {
    //   closeModal()
    // }
...
<template>
  ...
  <Modal :showModal="showModal" title="Vieux motard que jamais !" :close="closeModal">
    <div id="modal">
      <p>Salut, je suis le chatbot de l'application. Je suis là pour t'aider.</p>
      <p>Vous pouvez a tout moment mettre fin ou redémarrer la conversation en écrivant : "fin" ou "redémarrer"</p>
      <p>Comment je peux t'aider ?</p>
      <p>Je souhaiterais des l'entretien sur mon véhicule</p>
      <p>Je souhaite des informations sur les véhicules</p>
      <p>Je souhaite des informations de contact</p>
    </div>
    <input v-model="text" type="text" id="input">
    <button v-on:click="onclick" id="button">OK</button>
  </Modal>
